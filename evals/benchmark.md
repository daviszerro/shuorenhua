...
|------|-------|------|-------|-------------|
| chat | SF-01, SF-06, SF-11, SF-17, SF-29, SF-31, SF-36, SF-37, SF-38, SNF-07, SNF-11 | - | SF-19, SNF-16 | - |
...
| public-writing | SF-03, SF-05, SF-08, SF-10, SF-12, SF-13, SF-16, SF-20, SF-28, SF-32, SF-33, SF-35, SF-42, SNF-12, SNF-24, SNF-25, SNF-27 | SF-34, SF-39, SF-40, SF-41, SNF-26, SNF-29, SNF-30, SNF-31, SNF-32 | SF-14 | - |
| code-context | - | - | - | SNF-22 |
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中开场套话“值得注意的是”、谄媚“好问题！”、“让我来为你详细解释”。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 English Tier 1 copula avoidance `serves as a testament` and significance inflation `pivotal`, `transformative`, `evolving landscape`.

### SF-08 | public-writing | 戏剧化碎句 + 金句感
> 三年。两个团队。一个目标。当我们回头看这段旅程，每一步都充满了不可磨灭的意义。这不仅仅是一个产品，更是一种信念的传承。

**预期**：诊断为 AI 生成 (高置信度)。应当命中结构反模式“二元对比”、“戏剧化碎句”，以及 Tier 1 情感夸张词。

### SF-09 | status | 被动语态堆砌
> 系统被全面优化后，性能被显著提升，用户体验被大幅改善，安全性被进一步加强。

**预期**：诊断为 AI 生成 (高置信度)。应当命中结构反模式“被动语态堆砌”与 Tier 2/3 修饰词的饱和使用。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 English Tier 1 sycophantic opener `Great question!`, meta-commentary `In this essay we will explore`, and filler `Let's dive in!`.

### SF-11 | chat | 工程师腔 / 调试腔
> 我已经把差异收窄了，根因基本坐实，和我刚抓到的现象对上了。接下来做一个更硬的排除法，稳稳把问题兜住，落盘之后就能收口了。

**预期**：诊断为 AI 生成 (高置信度)。应当命中 Tier 1 虚假工程师调试腔“差异收窄”、“根因坐实”、“对上”、“更硬的排除法”、“稳稳兜住”、“落盘”、“收口”。

### SF-12 | public-writing | 小红书 AI 腔
> 姐妹们！今天给大家拆解一个保姆级干货！真的绝绝子！谁懂啊，这个工具狠狠提升了我的效率！强烈建议收藏！划重点：避坑指南在最后！

**预期**：诊断为 AI 生成 (高置信度)。应当命中 Tier 1 自媒体 AI 腔“姐妹们”、“保姆级”、“干货”、“绝绝子”、“谁懂啊”、“狠狠”。

### SF-13 | public-writing | 正能量收尾 + 鸡汤
> 诚然，AI 技术仍面临诸多挑战。但与其抗拒变化，不如积极拥抱这个充满无限可能的时代。只有不断学习、勇于创新，才能在未来的浪潮中乘风破浪。让我们拭目以待！

**预期**：诊断为 AI 生成 (高置信度)。应当命中 Tier 1 鸡汤收尾“与其……不如积极拥抱……”、“只有……才能……”、“拭目以待”。

### SF-14 | chat | 语域混搭
> 诚然，这个 feature 的实现确实存在一定的技术复杂度。不过说白了就是绝绝子！我们需要进一步深入探讨其底层逻辑，稳稳把核心链路兜住。综上所述，建议收藏。

**预期**：诊断为 AI 生成 (高置信度)。应当命中显著的语域混搭（学术腔“诚然”与网感“绝绝子”、工程师腔“兜住”等混杂）。

### SF-15 | status | 句长均匀（节奏单调）
> 本次更新优化了系统的整体性能。我们改进了数据库的查询效率。前端页面的加载速度得到了提升。用户反馈的体验问题已经得到解决。后续将持续关注系统的稳定性。

**预期**：诊断为 AI 生成 (中置信度)。应当命中结构反模式“句长过于均匀”及 Tier 2/3 修饰词的高频堆砌。

### SF-16 | public-writing | 基础中文套话骨架
> 真正的竞争力不是功能堆砌，而是体验细节。最后比拼的是执行效率。归根结底，关键在于团队协同。

**预期**：诊断为 AI 生成 (高置信度)。应当命中结构反模式“价值拔高骨架”（真正的竞争力不是...而是...、归根结底）。

### SF-17 | chat | 模式变体归并
> 我先把问题扒开，现象也拽出来了。再补一刀，把这轮链路锁住，基本就闭环了。

**预期**：诊断为 AI 生成 (高置信度)。应当匹配为“庸医问诊腔 / 暴力动作腔 / 调试腔”模式变体（“扒开”、“拽出来”、“补一刀”、“闭环”）。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中结构反模式“二元对比”、“空泛总结”，及 Tier 1 词汇“研究表明”、“稳稳兜住”。
...
**预期**：诊断为 AI 生成 (高置信度)。指示应当只诊断引号内的正文，命中 Tier 1 词汇“结论先说”、“系统性重塑”、“扒开”、“补一刀”、“稳稳兜住”、“质的跃迁”。
...
**预期**：诊断为 AI 生成 (高置信度)。指示应当只诊断 Python 注释/docstring 中的文字，命中 Tier 1 词汇“全面优化”、“显著提升”、“整体性能”。
...
**预期**：诊断为 AI 生成 (高置信度)。指示应当只诊断 commit message，命中 Tier 1 词汇“打造”、“赋能”、“降本增效”、“闭环”、“显著提升”。
...
**预期**：诊断为 AI 生成 (高置信度)。指示应当只诊断 Javascript 注释，命中 English Tier 1 词汇 `groundbreaking`, `serves as a testament`, `leveraging`, `unprecedented`.
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 English Tier 1 无源引用 `Studies show` and `Experts say`.

### SF-21 | status | 无源引用在保守场景里的处理
> 数据显示，这次改版显著提升了留存率。业内人士认为，这个方向已经验证可行，后续只要继续投入就能稳定放大收益。

**预期**：诊断为 AI 生成 (高置信度)。应当命中 Tier 1 无源引用“数据显示”、“业内人士认为”，且指出在 status 场景下属于无事实支撑的空论。
...
**预期**：诊断为 AI 生成 (中/高置信度)。应当匹配结构反模式“二元对比”（不是……而是……），且在诊断报告中对具体事实数字（4月12日、留存率、王宁等）进行“免误杀与排噪放行”分析。
...
**预期**：诊断为 AI 生成 (中置信度)。应当命中 Tier 1 调试腔“系统性治理”、“稳稳兜住”；且对路径命令 `bin/migrate`、版本号 `v2.3.1` 等做“免误杀与排噪放行”分析。
...
**预期**：诊断为 AI 生成 (中置信度)。指示应当只诊断 Go 注释，命中 Tier 1 调试腔“系统性治理”、“稳稳兜住”；且对版本/数字等排噪放行。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 README intro 场景中的价值口号堆砌（“全面重塑开发范式”、“先进的规则体系”、“价值闭环”）。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 release note 场景中的发布宣言与空泛升级叙事（“面向真实场景的系统性升级”、“全新的能力矩阵”、“共同见证”）。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 forum-post 场景中的方法论闭环与商业对外稿腔调（“系统性重塑”、“痛点”、“方法论闭环”）。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 issue-reply 场景中的客服式安抚与推销式助手腔（“感谢宝贵反馈”、“充分接住了”、“如果你愿意”）。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中路径正确性认证/夸奖（“已经走在正确的路上了”、“走得很稳”）与庸医问诊腔变体（“彻底掰开说清楚”）。

### SF-37 | chat | 对人本身发证书 / 身份能力认证夸奖变体
> 你能问到这个触及核心的问题，说明你已经超越绝大部分人了。如果你愿意，我可以给你更完整的方案 —— 你已经具备做这件事的实力了。

**预期**：诊断为 AI 生成 (高置信度)。应当命中身份能力认证夸奖（“超越绝大部分人了”、“具备做这件事的实力了”）。

### SF-38 | chat | 庸医问诊腔变体（掰开 / 掰扯清楚）
> 我帮你把这个事情掰扯清楚，先把它彻底掰开说清楚。

**预期**：诊断为 AI 生成 (高置信度)。应当命中庸医问诊腔变体（“掰扯清楚”、“彻底掰开说清楚”）。
...
**预期**：诊断为 AI 生成 (中置信度)。应当命中结构反模式“价值拔高骨架”与 Tier 1 开场套话，同时对包含正常承接和作者节奏的重复长句进行排噪说明。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中结构反模式“不仅是……更是……”、“与其说……不如说……”，即使其以长文形式存在。
...
**预期**：诊断为 AI 生成 (高置信度)。应当命中 Bounded 场景中的三句整句空话（谄媚开场、无源引用、价值拔高收尾），且对带数字的实句和正常排比句进行排噪说明。

### SF-42 | public-writing / README | 自我宣传里的“做快做稳”残味
> AI 工具很多，真正能帮开发者把活做快、做稳的并不多。这个项目做的，就是把模型写出来的套话和表演感压下去，让结果更像人写的。

**预期**：诊断为 AI 生成 (高置信度)。应当命中自我宣传空夸“把活做快、做稳”、“更像人写的”。
...
**预期**：安全放行 (低置信度)。放行理由："navigates" and "traversing" are literal technical descriptions of graph algorithms, not business jargon.
...
**预期**：安全放行 (低置信度)。放行理由：变更日志的简洁风格不需要改写，句式本身就是直接的。

### SNF-07 | chat | 正常的 Tier 3 低频使用
> 这个功能很重要，上线前一定要确保测试覆盖到位。

**预期**：安全放行 (低置信度)。放行理由："重要""确保"是 Tier 3 词，低频使用完全正常。

### SNF-08 | docs | 讨论术语本身
> 什么是"赋能"？在互联网行业中，这个词通常被用来指代"提供工具或能力让他人能做之前做不到的事"。但由于过度使用，它已经失去了具体含义。

**预期**：安全放行 (低置信度)。放行理由：当 Tier 1 词本身是讨论对象时，不应替换。
...
**预期**：安全放行 (低置信度)。放行理由：正常的代码注释，有具体参数（TTL 300 秒）和具体降级策略（Redis → 本地 LRU），不是 AI 腔。
...
**预期**：安全放行 (低置信度)。放行理由：具体、事实性的 commit message，有数据（20→100）和问题（504），不需要改写。

### SNF-19 | status | 已经直接的事实同步
> 4 月 12 日把连接池上限从 20 调到 100 后，504 错误率从 3.8% 降到 0.6%。今天继续观察 24 小时，再决定是否全量。

**预期**：安全放行 (低置信度)。放行理由：这段已经是直接的状态同步，重点全在时间、动作、数值和下一步上。即使 protected spans 很密，也不应该为了“更像人”再抛光一遍。

### SNF-20 | docs | 第二遍不该为了节奏改技术说明
> 在 `v2.4.0` 里，`retry_budget=0.2` 时默认开启指数退避；如果看到 `429 too many requests`，先把 `MAX_INFLIGHT=64` 调低再重试。

**预期**：安全放行 (低置信度)。放行理由：这已经是直接的技术说明，版本号、参数和报错都承载事实。第二遍不该为了“节奏更自然”去改写这种命令式说明，也不能动这些受保护片段。

### SNF-21 | status | 已经够直接时，第二遍应该停手
> 4 月 14 日补回 `source=campaign` 维度后，Android 次日留存从 28.7% 修正到 30.1%。王宁今晚再核一次报表，明早 10 点同步最终数。

**预期**：安全放行 (低置信度)。放行理由：这段已经把时间、动作、归属、数值和下一步说清楚了。第二遍不该再加口语化停顿、补解释句，或为了“更像人”改掉这种直接同步风格。

### SNF-33 | status | 有指标支撑的“稳定”不是自我宣传残味
> 6 月 28 日把 WebSocket idle 连接自动断开上线后，内存从 1.2GB 降到 520MB，服务稳定在 99.95% 以上。今天继续观察晚高峰，再决定是否全量。

**预期**：安全放行 (低置信度)。放行理由：这里的“稳定”挂在具体上线动作和指标上（`6 月 28 日`、`WebSocket idle`、`1.2GB`、`520MB`、`99.95%`），是状态同步里的事实描述，不是产品自夸或 README 宣传。v1.9.1 起 `做快、做稳 / 又快又稳 / 更快更稳` 只在自我宣传、项目介绍、营销式 README 等场景里按空泛效率承诺处理；有真实主体、指标或稳定性结果时应放行，不能机械删除或改写。
...
**预期**：安全放行 (低置信度)。放行理由：`接住` 的宾语是 `突发请求`，是技术语境里 handle / absorb 的直接表达，不承担姿态。v1.7.3 起 `接住` 已经按宾语判断：宾语是请求 / 流量 / 峰值时放行，不应因单词命中把代码注释里的技术动作改平。
...
**预期**：安全放行 (低置信度)。放行理由：这是证据充分的技术复盘，虽然包含 `root cause / 打满 / 指标 / 告警` 等工程语汇，但都承载了具体参数、动作和结果，不应为了“去 AI 味”而改平。
...
**预期**：安全放行 (低置信度)。放行理由：整段是具体的限流架构说明，`稳稳接住` 的宾语是 `上游峰值请求`，承载的是限流网关的技术动作，不是姿态层。配置值、阈值、指标名都是受保护片段。v1.7.3 起 `接住` 已按宾语判断：宾语是请求 / 流量 / 峰值时，`docs / code-context` 场景应放行，不应因 `稳稳接住` 短语命中把整段技术说明改平。

### SNF-28 | status | 技术语境里的"落盘"
> 我开始落盘了：把新的重构方案、阶段拆分、边界和 TODO 写进 planning-with-files 目录的三份文档，下一步按这套执行。

**预期**：安全放行 (低置信度)。放行理由：v1.8.3 起 `落盘 / 落 X` 按宾语判断，规则同 v1.7.3 的 `接住`。这里 `落盘` 的宾语是 `重构方案 / 阶段拆分 / 边界 / TODO` 这类具体技术对象，且能复述"写到哪里"（`planning-with-files 目录的三份文档`），是开发流程中实际的"写入磁盘 / 落地存档"动作，不是姿态层。文档目录名 `planning-with-files`、文档数量 `三份`、阶段词都是受保护片段，不应因 `落盘` 单词命中改平。
...
**预期**：安全放行 (低置信度)。放行理由：这里的“我想慢一点说”重复承担段落节奏和作者立场，不是空总结。`in-place` scope 下不应删掉重复句、合并第二段三个时间点，或把全文压成“作者认为 AI 过度整理会损失节奏”。可以句内清理个别过顺表达，但必须保留重复节奏。
...
**预期**：安全放行 (低置信度)。放行理由：`另外 / 与此同时 / 也就是说` 都挂在具体事实和判断上下文里，承担长文承接功能。即使命中连接词或总结提示信号，也不应在 `in-place` 下整句删除、并句或重排。只允许处理句内的拔高词，不能误杀正常转场。
...
**预期**：安全放行 (低置信度)。放行理由：测 `bounded` 删除清单的边界。`说到底` 是句首引导词，但删掉它之后整句仍是带信息的立场判断（作者真正看重什么），不是整句空话 —— 只能句内删 `说到底` 三字，整句不进删除清单。`第一次……第二次……` 是承担节奏的排比，且带具体信息（创业团队 / 大厂、半夜挂掉 / 上千万用户），更不能进清单。如果 `bounded` 把这几句当整句空话删掉或塞进删除清单，记 `❌`。
...
**预期**：安全放行 (低置信度)。放行理由：来源为 `results-v1.8.6` §4 记录的越界行为，用来钉住 `bounded` 防并句边界。第一句是商业黑话壳句，句内洗或进「建议删除（待确认）」清单即可；**不得与第二句合并成一句输出**，更不能把第二句改成总结句或软化成“明显提升效率”之类的新说法。第二句是具体数据句，`上个月`、`接进 CI`、`18 分钟`、`6 分钟` 必须逐字保留。
...
**预期**：安全放行 (低置信度)。放行理由：这段 README intro 已经说清楚“是什么、做什么、给谁用”，没有价值口号或发布宣言。`CLI`、`Redis`、`key dump`、`TTL` 都是必要术语，不应为了更口语而改掉。
...
**预期**：安全放行 (低置信度)。放行理由：这段 release note 已经按变更列表呈现，有版本号、文件名和具体动作。不要为了“更像人”改成叙事段落，也不要删掉路径、版本号或 case 数量。
...
**预期**：安全放行 (低置信度)。放行理由：这是正常社区复盘，有时间、文件名、样本范围、发现的问题和下一步。即使语气口语，也有具体经历支撑，不应改成正式公告或删掉 `scene-packs.md`、README、release note、SNF 等关键信息。
...
**预期**：安全放行 (低置信度)。放行理由：这条 issue 回复已经先确认问题、说明复现结果和下一步，没有客服式安抚。`bad case`、`docs`、`SNF` 都是 issue 语境里的必要术语，不应误杀。
...
**预期**：安全放行 (低置信度)。放行理由：这是在讨论收词策略，不是在正文里表演姿态。即使出现了已标记词，也应因为“被讨论 / 被引用”而放行。
